...
-- CONSULTA
--
-- IMPORTANTE - DOS USUARIOS DISTINTOS EJECUTAN ESTE SCRIPT:
-- Segun la tabla de usuarios del enunciado (seccion 2.1), la accion "Crear
-- indices" esta reservada UNICAMENTE para BDY1102_ET_FD (el dueno de las
-- tablas); BDY1102_ET_FD_DES nunca recibio ese privilegio (ni CREATE ANY
-- INDEX ni INDEX sobre estas tablas puntuales). Por lo tanto:
--   * Las secciones 1, 2 y 4 (consulta + EXPLAIN PLAN) se ejecutan
--     conectado como BDY1102_ET_FD_DES.
--   * La seccion 3 (CREATE INDEX) se debe ejecutar conectado como
--     BDY1102_ET_FD.
-- ============================================================================

-- NOTA IMPORTANTE: RESULTADO_SOLICITUD, DETALLE_PUNTAJE_SOLICITUD y
-- ANTECEDENTES_CLIENTE son propiedad de BDY1102_ET_FD. Ejecutando conectado
-- como BDY1102_ET_FD_DES, deben referenciarse calificadas con el esquema
-- dueno; de lo contrario Oracle las busca en el esquema del usuario
-- conectado y arroja ORA-00942 (aunque el privilegio SELECT este otorgado).
...
JOIN   BDY1102_ET_FD.DETALLE_PUNTAJE_SOLICITUD dp
       ON dp.run_cliente = rs.run_cliente
JOIN   BDY1102_ET_FD.ANTECEDENTES_CLIENTE ac
       ON (ac.numrun || '-' || ac.dvrun) = rs.run_cliente
WHERE  rs.ptje_final_sol > (SELECT AVG(ptje_final_sol) FROM BDY1102_ET_FD.RESULTADO_SOLICITUD)
ORDER  BY rs.ptje_final_sol DESC;


-- ----------------------------------------------------------------------------
-- 2) PLAN DE EJECUCION ANTES DE OPTIMIZAR (conectado como BDY1102_ET_FD_DES)
-- ----------------------------------------------------------------------------
EXPLAIN PLAN FOR
SELECT ac.apaterno,
       ac.amaterno,
...
-- CAMBIAR DE CONEXION: esta seccion se ejecuta conectado como BDY1102_ET_FD
-- (dueno de las tablas), unico usuario habilitado para crear indices segun
-- el enunciado. Al estar conectado como el propio dueno, no es necesario
-- calificar los nombres de tabla con el esquema.
-- ----------------------------------------------------------------------------
...
-- CAMBIAR DE CONEXION DE VUELTA: se ejecuta conectado como BDY1102_ET_FD_DES
-- (por eso las tablas vuelven a estar calificadas con el esquema dueno).
-- ----------------------------------------------------------------------------
EXPLAIN PLAN FOR
SELECT ac.apaterno,
       ac.amaterno,
