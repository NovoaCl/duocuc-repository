...
-- EVALUACION FINAL TRANSVERSAL - FORMA A
-- CASO: BECA PASANTIA DE PERFECCIONAMIENTO EN EL EXTRANJERO PARA DOCENTES DE
-- LA EDUCACION PUBLICA - DIREDUCA / EDUCAPP
--
...
-- PTJE_ANNOS_EXPERIENCIA, PTJE_PAIS_POSTULA, PTJE_PUEBLO_INDIGENA,
-- PUEBLO_INDIGENA, REGION, SERVICIO_LOCAL_EDUCP, DETALLE_PUNTAJE_POSTULACION,
-- RESULTADO_POSTULACION) deben existir previamente, creadas por
-- BDY1102_ET_FA al ejecutar script_crea_tablas_becas_docentes.sql (entregado
-- en semana 18, segun ANEXO_A_MODELO_BECAS_DOCENTES).
--
-- Orden de ejecucion:
--   1) SECCION A se ejecuta conectado como un usuario con privilegios de
--      administrador (ej. SYSTEM).
--   2) Luego se ejecuta script_crea_tablas_becas_docentes.sql conectado como
--      BDY1102_ET_FA (dueno del modelo).
--   3) Finalmente, la SECCION B de este script se ejecuta conectado como
--      BDY1102_ET_FA, ya que es el dueno de las tablas y debe otorgar los
...
-- ----------------------------------------------------------------------------
CREATE TABLESPACE TBS_BDY1102_FA_DATOS
  DATAFILE 'tbs_bdy1102_fa_datos01.dbf' SIZE 100M
  AUTOEXTEND ON NEXT 10M MAXSIZE 500M;

CREATE TABLESPACE TBS_BDY1102_FA_DES
  DATAFILE 'tbs_bdy1102_fa_des01.dbf' SIZE 50M
...
-- ----------------------------------------------------------------------------
CREATE USER BDY1102_ET_FA
  IDENTIFIED BY "Educapp_2026*FA"
  DEFAULT TABLESPACE TBS_BDY1102_FA_DATOS
  TEMPORARY TABLESPACE TEMP
  PROFILE PERF_BDY1102_ET_FA
  ACCOUNT UNLOCK;

ALTER USER BDY1102_ET_FA QUOTA UNLIMITED ON TBS_BDY1102_FA_DATOS;

CREATE USER BDY1102_ET_FA_DES
  IDENTIFIED BY "Educapp_2026*DES"
  DEFAULT TABLESPACE TBS_BDY1102_FA_DES
  TEMPORARY TABLESPACE TEMP
  PROFILE PERF_BDY1102_ET_FA_DES
  ACCOUNT UNLOCK;

ALTER USER BDY1102_ET_FA_DES QUOTA 200M ON TBS_BDY1102_FA_DES;

-- Se obliga a cambiar la clave en el primer inicio de sesion.
ALTER USER BDY1102_ET_FA PASSWORD EXPIRE;
ALTER USER BDY1102_ET_FA_DES PASSWORD EXPIRE;

-- ----------------------------------------------------------------------------
-- A.4 PRIVILEGIOS DE SISTEMA
-- ----------------------------------------------------------------------------

-- BDY1102_ET_FA: dueno del modelo. Nota: crear un indice sobre una tabla
...
-- BDY1102_ET_FA_DES. Se debe confirmar/ajustar con el enunciado completo de
-- semana 18.
GRANT CREATE PROCEDURE TO BDY1102_ET_FA;

-- BDY1102_ET_FA_DES: usuario generico del desarrollador. Requiere crear
...
-- ############################################################################
-- SECCION B: PRIVILEGIOS DE OBJETOS (conectado como BDY1102_ET_FA, una vez
-- ejecutado script_crea_tablas_becas_docentes.sql)
-- ############################################################################
...
-- SQL sueltas ejecutadas dentro de la sesion). Como BDY1102_ET_FA_DES debe
...
GRANT SELECT ON ANTECEDENTES_LABORALES       TO BDY1102_ET_FA_DES;
GRANT SELECT ON ANTECEDENTES_PERSONALES      TO BDY1102_ET_FA_DES;
GRANT SELECT ON INSTITUCION                  TO BDY1102_ET_FA_DES;
GRANT SELECT ON PAIS                         TO BDY1102_ET_FA_DES;
GRANT SELECT ON PASANTIA_PERFECCIONAMIENTO   TO BDY1102_ET_FA_DES;
GRANT SELECT ON POSTULACION_PASANTIA_PERFEC  TO BDY1102_ET_FA_DES;
GRANT SELECT ON PTJE_ANNOS_EXPERIENCIA       TO BDY1102_ET_FA_DES;
GRANT SELECT ON PTJE_PAIS_POSTULA            TO BDY1102_ET_FA_DES;
GRANT SELECT ON PTJE_PUEBLO_INDIGENA         TO BDY1102_ET_FA_DES;
GRANT SELECT ON PUEBLO_INDIGENA              TO BDY1102_ET_FA_DES;
GRANT SELECT ON REGION                       TO BDY1102_ET_FA_DES;
GRANT SELECT ON SERVICIO_LOCAL_EDUCP         TO BDY1102_ET_FA_DES;

-- ----------------------------------------------------------------------------
-- B.2 MANTENCION DE RESULTADOS (SELECT / INSERT / UPDATE / DELETE)
-- ----------------------------------------------------------------------------
-- Estas tablas son exclusivas de BDY1102_ET_FA_DES (nadie mas las usa), por
...
GRANT SELECT, INSERT, UPDATE, DELETE ON DETALLE_PUNTAJE_POSTULACION TO BDY1102_ET_FA_DES;
...
--   otorgar los INSERT/SELECT correspondientes a BDY1102_ET_FA_DES.
...
--   BDY1102_ET_FA (AUTHID DEFINER) con EXECUTE otorgado a BDY1102_ET_FA_DES,
--   en vez de otorgar DROP ANY TABLE.
-- * Con las reglas de negocio de puntaje (pueblo indigena, anos de
